...
func (s *stub) scenario() {
	<-s.connected
	s.events <- `{"type":"message.part.updated","properties":{"part":{"sessionID":"ses_e2e"},"delta":"working"}}`
	s.events <- `{"type":"permission.updated","properties":{"id":"perm_e2e","type":"bash","sessionID":"ses_e2e","messageID":"m1","title":"run","metadata":{},"time":{"created":0}}}`
	<-s.permCh
	s.events <- `{"type":"message.part.updated","properties":{"part":{"sessionID":"ses_e2e"},"delta":"done"}}`
