...
- Privacy Policy URL: `https://ironrain.app/privacy`
- Support URL: `https://ironrain.app/support`
  ...
  extensionless form directly with a 200 — the `.html` form 307s to it — so these are the addresses
  to register: an externally committed URL should not depend on a redirect. CI fails the deploy if
  either stops returning 200.
