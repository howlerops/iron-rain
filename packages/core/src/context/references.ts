 ...
 */
export function parseReferences(input: string, cwd: string, contextDirs: string[] = []): ParsedInput {
...
function resolveToken(raw: string, cwd: string, contextDirs: string[]): ResolvedReference | null {
  // @git:cmd
...
function resolveFile(filePath: string, cwd: string, contextDirs: string[]): ResolvedReference | null {
    ...
    try {
      ...
      if (stat.size > MAX_FILE_SIZE) {
        return {
          type: 'file',
          path: abs,
          content: `<file path="${relative(cwd, abs)}">\n(file too large: ${Math.round(stat.size / 1024)}KB, limit ${MAX_FILE_SIZE / 1024}KB)\n</file>`,
        };
      }
...
function resolveDirectory(dirPath: string, cwd: string, contextDirs: string[]): ResolvedReference | null {
  const searchPaths = [cwd, ...contextDirs];
...
function resolveImage(imagePath: string, cwd: string, contextDirs: string[]): ResolvedReference | null {
  const searchPaths = [cwd, ...contextDirs];
    ...
    try {
      ...
      if (stat.size > MAX_IMAGE_SIZE) {
        return {
          type: 'image',
          path: abs,
          content: `<image path="${relative(cwd, abs)}">\n(image too large: ${Math.round(stat.size / 1024 / 1024)}MB, limit ${MAX_IMAGE_SIZE / 1024 / 1024}MB)\n</image>`,
        };
      }
...
function resolveGit(cmd: string, cwd: string): ResolvedReference | null {
