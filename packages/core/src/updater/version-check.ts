...
const NPM_REGISTRY_URL = 'https://registry.npmjs.org/@howlerops/iron-rain-cli/latest';
...
export async function runDiagnostics(config?: any): Promise<DoctorCheck[]> {
  ...
  checks.push({
    ...
    message: hasBun ? `v${(globalThis as any).Bun.version}` : 'Not running in Bun (TUI mode unavailable)',
  });
  ...
  try {
    ...
    checks.push({
      ...
      message: configPath ? `Found: ${configPath}` : 'No config file found (using defaults)',
    });
  } catch (e) {
    checks.push({
      name: 'Config File',
  ...
  if (config?.slots) {
    ...
    for (const [slotName, slotConfig] of Object.entries(config.slots) as [string, any][]) {
      try {
        ...
        checks.push({
          ...
          message: available ? `${slotConfig.model} ready` : `${slotConfig.model} not available`,
        });
      } catch (e) {
        checks.push({
          name: `Bridge: ${slotName} (${slotConfig.provider})`,
