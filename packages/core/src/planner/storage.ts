...
export class PlanStorage {
  save(plan: Plan): void {
    ...
    const planJson = { ...plan };
    fs.writeFileSync(path.join(dir, 'plan.json'), JSON.stringify(planJson, null, 2));
  ...
  list(): Array<{ id: string; title: string; status: string; createdAt: number }> {
    ...
    try {
      return fs.readdirSync(dir)
        ...
        .map(entry => {
          ...
          return plan
            ? { id: plan.id, title: plan.title, status: plan.status, createdAt: plan.createdAt }
            : null;
        })
