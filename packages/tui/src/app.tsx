...
import { checkForUpdate, loadConfig } from "@howlerops/iron-rain";
...
function checkProviderHealth(config?: IronRainConfig): string[] {
  ...
  for (const [slotName, slotConfig] of Object.entries(slots)) {
    ...
    const resolvedKey = apiKey ?? providers[provider]?.apiKey ?? undefined;

    if (!resolvedKey) {
      warnings.push(
        `**${slotName}** slot: No API key for ${provider}. Run /settings to configure.`,
      );
      continue;
    }
    ...
    if (resolvedKey.startsWith("env:")) {
      ...
      if (!process.env[envVar]) {
        warnings.push(
          `**${slotName}** slot: Environment variable \`${envVar}\` is not set.`,
        );
      }
    }
  }
...
export function App(props: AppProps) {
  const hasValidConfig = !!(
    props.config?.slots && Object.keys(props.config.slots).length > 0
  );
